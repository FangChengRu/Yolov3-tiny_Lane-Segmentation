...
def main():
    ...
    model = get_net(export=False).to(device)
    # print("load finished")
    ...
    Encoder_para_idx = [str(i) for i in range(0, 13)]
    Det_Head_para_idx = [str(i) for i in range(13, 23)]
    Da_Seg_Head_para_idx = [str(i) for i in range(23, 34)]
    Ll_Seg_Head_para_idx = [str(i) for i in range(34, 45)]
    ...
    if rank in [-1, 0]:
        ...
        if os.path.exists(cfg.MODEL.PRETRAINED_DET):
            ...
            det_idx_range = [str(i) for i in range(0, 16)]
